...
//why the hell is there no standard library function for log2?!
int log2 (int val) {
    int ret = 0;
    while (val > 1) {
        val >>= 1;
...
Platform::String^ Calculate (int ip1, int ip2, int ip3, int ip4, int sub1, int sub2, int sub3, int sub4)  //main function of the program.  where the program starts
{
...
Platform::String^ returnText = ""; //string for holding all text results displayed
...
intOct1 = ip1;
intOct2 = ip2;		//initialize ip values
intOct3 = ip3;
intOct4 = ip4;

intMask = log2(sub1 + 1);
intMask = intMask + log2(sub2 + 1);
intMask = intMask + log2(sub3 + 1);
intMask = intMask + log2(sub4 + 1);  //add up all subnet bits 
...
//printf ("Invalid IP address, out of range, or mask out of range"); //Print an error message and exit ...
	return "Invalid IP address, out of range, or mask out of range";
...
//if (numBitsBor<2) { //check that at least 2 bits must be borrowed to create a valid subnet mask
    ...
    //exit (0); //exit the program
	//return "Not enough bits borrowed, Invalid mask";
//}
...
//returnText = "Class: " + classType.ToString();   //can't use this because ToString returns an integer..

//workaround for this whole Platform::String business of not being able to add character to string
Platform::String^* ClassChar;
if (classType == 'A') {
	Platform::String^ x = "A";
	ClassChar = &x;
} else if (classType == 'B') {
...
//forms return information for class and network information
Platform::String^ Class = "Class: " + *ClassChar + "\n";
Platform::String^ NetworkID = "Network ID: " + nOct1 + "." + nOct2 + "." + nOct3 + "." + nOct4 + "\nNetwork Broadcast Address: " + bOct1 + "." + bOct2 + "." + bOct3 + "." + bOct4 + "\n";
...
Platform::String^ SubnetMask = "Subnet Mask: " + mOct1 + "." + mOct2 + "." + mOct3 + "." + mOct4 + "\n";
...
Platform::String^ SubnetAddress = "Subnet Address: " + sOct1 + "." + sOct2 + "." + sOct3 + "." + sOct4 + "\n";
Platform::String^ SubnetBroadcast = "Subnet Broadcast Address: " + sBOct1 + "." + sBOct2 + "." + sBOct3 + "." + sBOct4 + "\n";
...
for (index=1;index<netBits;index++) {  // Loop for the number of bits borrowed -1 times.
    numSubnets=numSubnets*2; //Multiply the number of subnets to 2;
    }
numUseSubs=netBits-2; //Subtract 2 from the number of subnets to find the number of usable subnets.
...
totalHosts=(useHostPerSub + 2)*(numUseSubs + 2); // Calculate the total number of hosts by multiplying the number of hosts per subnet by the number of subnets.
...
Platform::String^ BitsBorrowed = "Number of bits borrowed: " + numBitsBor + "\n";
//printf ("Number of subnets created: %i ; usable subnets: %i\n", numSubnets, numUseSubs); // Print the number of subnets created and the number
                                                                                      //of usable subnets.
Platform::String^ NumSubnetsCreated = "Number of subnets created: " + numSubnets + "; usable subnets: " + numUseSubs + "\n";
...
Platform::String^ NumHosts = "Number of hosts per subnet: " + numHostsPerSub + "; usable hosts per subnet " + useHostPerSub + "\n";
//printf ("Total Hosts: %i ; Total usable Hosts: %i\n",totalHosts, totalUseHosts); // Print the total hosts and the total usable hosts.

Platform::String^ TotalHosts = "Total Hosts: " + totalHosts + "; total usable hosts: " + totalUseHosts + "\n";
//printf ("\n\nPress enter to continue");// Prompt user to press enter when done

//index = input (strIP, 255);        //Wait for user to press enter.

//sums up all the return strings and returns everything
return Class + NetworkID + SubnetMask + SubnetAddress + SubnetBroadcast + BitsBorrowed + NumSubnetsCreated + NumHosts + TotalHosts;
...
//created this for the ridiculous reason that there is no way to parse a Platform::String into an integer
int integerFromString(String^ input)
{
	int returnInt = 0;

	//workaround for manipulating input strings
	//because Platform::String^ is immutable
	std::wstring inputwstring = input->Data();

	
	if (input->Length() > 3) {
		return 500;   //error code for invalid imput, each field cannot be more than 3 characters long
	} else if (input->Length() == 3) {
		returnInt += (inputwstring.at(0) - '0') * 100;
		returnInt += (inputwstring.at(1) - '0') * 10;
		returnInt += (inputwstring.at(2) - '0');
	} else if (input->Length() == 2) {                        //converts string of digits to an integer
		returnInt += (inputwstring.at(0) - '0') * 10;
		returnInt += (inputwstring.at(1) - '0');
	} else {
		returnInt += (inputwstring.at(0) - '0');
	}

	return returnInt;
}

void SubnetCalc::MainPage::buttonCalculate_Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
	int s1, s2, s3, s4, m1, m2, m3, m4;
	s1 = integerFromString(textboxIP1->Text);
	s2 = integerFromString(textboxIP2->Text);     //converts IP textbox inputs into strings
	s3 = integerFromString(textboxIP3->Text);
	s4 = integerFromString(textboxIP4->Text);

	m1 = integerFromString(textboxSubnet1->Text);
	m2 = integerFromString(textboxSubnet2->Text);
	m3 = integerFromString(textboxSubnet3->Text);   //converts subnet mask textbox inputs into strings
	m4 = integerFromString(textboxSubnet4->Text);

	//calculate function and output to textblock below input box
	textblockOutput->Text = Calculate(s1, s2, s3, s4, m1, m2, m3, m4);
}
